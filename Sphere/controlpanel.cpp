...
ControlPanel::ControlPanel(QWidget *parent) : QWidget(parent)
{
    ...
    xRotateControlBlock = new ControlBlock();
    connect(xRotateControlBlock, &ControlBlock::valueChanged, this, &ControlPanel::xRotateChanged);
    xRotateControlBlock->setLabel("Rotate x");
    xRotateControlBlock->setRange(-400, 400);
    ...
    yControlBlock->setRange(-400, 400);

    yRotateControlBlock = new ControlBlock();
    connect(yRotateControlBlock, &ControlBlock::valueChanged, this, &ControlPanel::yRotateChanged);
    yRotateControlBlock->setLabel("Rotate y");
    yRotateControlBlock->setRange(X_MIN, X_MAX);
    ...
    fillCheckbox = new QCheckBox("Bilinear", this);
    connect(fillCheckbox, &QCheckBox::clicked, this, &ControlPanel::filtrationChanged);

    auto layout = new QGridLayout();
    layout->addWidget(xControlBlock, 0, 0);
    layout->addWidget(yControlBlock, 1, 0);
    layout->addWidget(scaleControlBlock, 2, 0);
    layout->addWidget(xRotateControlBlock, 3, 0);
    layout->addWidget(yRotateControlBlock, 4, 0);
    layout->addWidget(fillCheckbox, 5, 0);
